...
export const InspectorModal: React.FC<InspectorModalProps> = ({
  ...
  const [loupePos, setLoupePos] = useState({ x: 0, y: 0, px: 0, py: 0 });
      ...
      >
        ...
        >
          <div className="flex items-center gap-3 truncate">
            <h3 className="font-semibold text-sm sm:text-base truncate" title={item.name} style={{ color: 'var(--c-text)' }}>
              {item.name}
            </h3>
            <span
        ...
        <div className="flex-1 overflow-y-auto grid grid-cols-1 lg:grid-cols-12 gap-0">
          ...
          >
            ...
            >
              ...
              {/* FLOATING 100% MAGNIFIER LOUPE */}
              {zoomLoupeEnabled && (
                <div
                  className="pointer-events-none absolute w-36 h-36 rounded-full overflow-hidden z-30"
                  style={{
                    left: `${loupePos.x - 72}px`,
                    top: `${loupePos.y - 72}px`,
                    border: '2px solid var(--c-brand-yellow)',
                    boxShadow: '0 8px 24px rgba(0,0,0,0.5)',
                    backgroundImage: `url(${compUrl})`,
                ...
                >
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className="w-2 h-2 rounded-full" style={{ background: 'rgba(246, 184, 60, 0.8)' }} />
                  </div>
                </div>
              )}
